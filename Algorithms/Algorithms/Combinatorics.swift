//
//  Combinatorics.swift
//  Swift-Alghoritms-Club
//
//  Created by Petro Korienev on 1/2/18.
...
extension Array where Element: Comparable {
    ...
    func permutationsOverMutations() -> [[Element]] {
        var copyOfSelf = self
        var result = Array<[Element]>(repeating: copyOfSelf, count: count.factorial)
        return result.map { _ in defer { let _ = copyOfSelf.permute() }; return copyOfSelf }
    }
    ...
    func permutationsOverMutationsFunctionalOptimized4() -> [[Element]] {
        var copyOfSelf = self
        var result = Array<[Element]>(repeating: copyOfSelf, count: count.factorial)
        return result.map { _ in defer { let _ = copyOfSelf.permuteFunctionalOptimized4() }; return copyOfSelf }
    }
    ...
    func permutationsConcurrentUnsafePointer(concurrentThreads: Int) -> [[Element]] {
        let result = Array<[Element]>.init(repeating: self, count: count.factorial)
        result.withUnsafeBufferPointer { resultPtr in
            let resultPtr = UnsafeMutablePointer(mutating: result)
            let seeds: [[Element]] = (0..<count).map {
                var copyOfSelf = self
                return [copyOfSelf.remove(at: $0)] + copyOfSelf
            }
            ...
            let seedSplits = seeds.split(by: splitSize)
            let group = DispatchGroup()
            let seedBufferSize = (count - 1).factorial
            seedSplits.enumerated().forEach { index, arrayOfSeeds in
                DispatchQueue.global(qos: .userInteractive).async(group: group) {
                    arrayOfSeeds.forEach { (seed) in
                        seed.unsafePermutationBackgroundWorker(
                            buffer: resultPtr.advanced(by: index * seedBufferSize),
                            iterationsCount: seedBufferSize
                        )
                    }
                }
            }
    ...
    private func unsafePermutationBackgroundWorker(
                    buffer: UnsafeMutablePointer<[Element]>,
                    iterationsCount: Int
        )
    {
        var copyOfSelf = self
        (0..<iterationsCount).forEach {
            buffer[$0] = copyOfSelf
            let _ = copyOfSelf.permuteFunctionalOptimized4()
        }
    }
...
//extension Array where Element: Equatable {
//    func combinations(by m: Int) -> [[Element]] {
//        guard m != 0 else { return [] }
//        guard m != 1 else { return map { [$0] } }
